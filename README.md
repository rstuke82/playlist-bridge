...
Playlist Details has one Sync mode: Server schedule, Custom schedule, or Manual only. The single Sync Playlists task covers Server schedule playlists. Custom schedules run independently, even if the server task is disabled. Manual Sync Now and Sync Selected/Filtered work regardless of mode and do not move the next scheduled time. Overlapping scheduled syncs are skipped. The single Health Check task checks all registered playlists without changing Plex contents or matching state. Filters cycle through include, exclude, and off. Ready to Sync includes pending match changes and newly matched missing tracks.
...
- Missing has remembered source, playlist, Missing/LOST and Manually matched elsewhere filters, with the same filter/sort controls as Playlists and playlist details.
...
- `playlist-bridge.db`: registered playlists, sync policies, mappings, provenance, missing/ignored tracks, snapshots, sync history, artist aliases, notification settings/history where present, and health results/history.
...
Settings is organized into General, Plex, Matching, Ignored Tracks, Tasks, Backups, Logs and About. Web sync, health, analyze, add, and match-save operations enter a persistent SQLite queue. One worker executes jobs in the background; progress and results remain available after navigation or refresh. The Dashboard offers Sync All Now. The Playlists action uses selected rows or the filtered results and shows the affected count. Favorites and Auto Sync flags have been removed from the web interface.
...
## Playlist names and Plex descriptions

Rename Playlist changes the Bridge and linked Plex name, preserving it across syncs without changing the source playlist. Use Source Name restores source naming. When adding, an optional destination name distinguishes identically named playlists; identities are based on source IDs, never names.

After a verified successful sync, Plex keeps the original source description followed by one Playlist Bridge summary: source URL, last successful sync timestamp, and source/matched/missing/ignored counts. This section is replaced on subsequent successful syncs.
