...
export default function SongPreview({track}:{track:{title:string;artist:string;album?:string}}){
 ...
 return <section className="song-preview"><button type="button" aria-expanded={open} aria-controls={id} onClick={()=>{if(open)close();else{setOpen(true);if(!rows)void find()}}}>{open?'Close Song Preview':'Preview Song'}</button>
 {open&&<div id={id} className="song-preview-panel" ref={panel}><p className="muted">Choose a recording to hear. Listening does not change your match.</p>{busy&&<p role="status">Finding song previews…</p>}{error&&<p className="error" role="alert">{error}</p>}
 {selected&&<div className="song-preview-player"><iframe key={selected.id} title={`Apple Music preview: ${selected.title}`} src={selected.embed_url} allow="encrypted-media" sandbox="allow-scripts allow-same-origin allow-popups allow-popups-to-escape-sandbox"/><p><a href={selected.store_url} target="_blank" rel="noreferrer">Open {selected.title} on Apple Music ↗</a> · <button type="button" className="small-button" onClick={()=>setSelected(null)}>Stop Preview</button></p></div>}
