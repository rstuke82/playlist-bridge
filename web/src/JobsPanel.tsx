import PlaylistScheduleTasks from './PlaylistScheduleTasks'
...
export function JobHistory({jobs,refresh}:{jobs:Job[];refresh:()=>Promise<void>}){
 ...
 return <section className="panel"><h2>Job history</h2>{error&&<ErrorNotice error={error}/>}{data?.rows.map((j:Job)=><JobRow key={j.id} job={j} refresh={refresh}/>)}{!data&&!error&&<p>Loading history…</p>}{data&&!data.total&&<p>No jobs yet.</p>}{data?.total>0&&<div className="actions"><button disabled={!offset} onClick={()=>setOffset(Math.max(0,offset-50))}>Previous</button><span>{offset+1}–{Math.min(offset+50,data.total)} of {data.total}</span><button disabled={offset+50>=data.total} onClick={()=>setOffset(offset+50)}>Next</button></div>}</section>
}
export function JobRow({job}:{job:Job;refresh?:()=>Promise<void>}){
 return <article className="job-row"><div className="panel-head"><div><strong>{actionName(job.action)} · {job.result?.partial_success?'Partially completed':job.status}</strong><p>{displayText(job.progress)}</p><small>{stamp(job.started_at||job.created_at)} · {job.payload.url||job.payload.title||({all:'All playlists',favorites:'Favorites',automatic:'Auto Sync playlists'}[job.payload.scope as string])||(['backup','check_updates','restore_backup'].includes(job.action)?'Maintenance':'Selected playlists')}</small></div><button onClick={()=>showActivity(job.id)}>{activeJob(job)?'View live activity':'View details'}</button></div>{job.error&&<ErrorNotice error={displayText(job.error)}/>}</article>
}
