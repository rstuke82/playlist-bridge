...
export default function App(){
  ...
  <main><header><div><h1><Brand/>{track?'Track details':detail?'Playlist details':page[0]?.toUpperCase()+page.slice(1)}</h1></div></header>
    ...
    {page==='activity'&&<><div className="actions"><a href="#activity">Jobs</a><a href="#activity/downloads">Downloads</a></div>{route.split('/')[1]==='downloads'?<LidarrDownloads albumKey={decodeURIComponent(route.split('/')[2]||'')}/>:<><Activity jobs={jobs} jobId={route.split('/')[1]||''}/><JobHistory jobs={jobs} refresh={refresh}/></>}</>}
...
function Missing({tracks,loaded,enqueue,refresh}:{tracks:MissingTrack[];loaded:boolean;enqueue:Enqueue;refresh:()=>Promise<void>}){
 ...
 {ignore&&<IgnoreDialog track={ignore} close={()=>setIgnore(null)} done={async()=>{await refresh();setError('Ignore queued. It will be saved after current activity finishes; Plex changes apply on the next sync.')}}/>}
 </section>
}
