...
function Missing({tracks,loaded,enqueue,refresh}:{tracks:MissingTrack[];loaded:boolean;enqueue:Enqueue;refresh:()=>Promise<void>}){
 ...
 return <section className="panel"><h2>Missing tracks</h2><p className="muted">An occurrence is one appearance of a track. Expand the counts to see its playlists. Filters combine across categories; choices within a category include either option.</p><div className="list-controls"><div className="toolbar-icons"><FilterMenu choices={choices} value={filters} onChange={setFilters} labels={labels}/><SortMenu choices={{'title:asc':'Name A–Z','title:desc':'Name Z–A','last_checked:desc':'Last checked: newest','last_checked:asc':'Last checked: oldest','occurrence_count:desc':'Most occurrences','occurrence_count:asc':'Fewest occurrences','playlist_count:desc':'Most playlists','playlist_count:asc':'Fewest playlists'}} value={sort} onChange={setSort}/><button className="icon-button" title="Refresh missing tracks" aria-label="Refresh missing tracks" onClick={()=>refresh().catch(e=>setError(e.message))}><Icon name="refresh"/></button></div><span className="list-count">{rows.length} of {tracks.length} tracks</span></div><div className="list-secondary"><input aria-label="Filter missing tracks" placeholder="Filter by track, artist or album" value={query} onChange={e=>setQuery(e.target.value)}/></div><FilterChips value={filters} onChange={setFilters} labels={labels}/>{error&&<p role="status">{error}</p>}
 {rows.map((t,i)=><article className="playlist-card" key={`${t.title}-${t.artist}`}><div className="panel-head"><div><a className="track-title" href={trackLink(t)}>{t.title}</a><p>{t.artist} · {t.album||'N/A'}</p></div><div className="actions"><button onClick={()=>setSelected(t)}>Review Match</button><button onClick={()=>setIgnore(t)}>Ignore</button></div></div>{!!t.manual_matches_elsewhere?.length&&<p><a href={trackLink(t)}>Manually matched elsewhere · Review saved selections ({t.manual_matches_elsewhere.length})</a></p>}<small>Last checked by sync/match: {stamp(t.last_checked)}</small><details><summary>{t.occurrence_count} occurrences in {t.playlist_count} playlists</summary>{t.memberships.map(m=><p key={m.key}><a href={link(m.key)}>{m.name}</a> · {m.count} occurrence(s)</p>)}</details></article>)}{!loaded?<p role="status">Loading missing tracks…</p>:!rows.length&&<div className="empty-state"><p>{query||filters.length?'No missing tracks match your filters.':'No missing tracks. You’re all caught up.'}</p>{(query||filters.length>0)&&<button onClick={()=>{setQuery('');setFilters([])}}>Clear Filters</button>}</div>}
