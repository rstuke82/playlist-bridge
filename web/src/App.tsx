...
export default function App(){
  ...
  const page=route.startsWith('playlist/')?'playlists':route.split('/')[0]
  ...
  return <div className="shell"><aside><div className="brand">Playlist Bridge</div><nav>{['dashboard','playlists','missing','search'].map(p=><button className={page===p?'active':''} key={p} onClick={()=>{location.hash=p}}>{p[0].toUpperCase()+p.slice(1)}</button>)}</nav><div className="sidebar-bottom"><button className={page==='settings'?'active':''} onClick={()=>{location.hash='settings'}}>Settings</button><div className="sidebar-version">{health.release_name||'Playlist Bridge 2.0 Beta 6'}</div></div></aside>
  <main><header><div><h1>{detail?'Playlist details':page[0]?.toUpperCase()+page.slice(1)}</h1></div></header>
...
function Missing({tracks,loaded,enqueue,refresh}:{tracks:MissingTrack[];loaded:boolean;enqueue:Enqueue;refresh:()=>Promise<void>}){
 ...
 {selected&&<MatchPicker track={selected} onClose={()=>setSelected(null)} onSave={async(c,keys)=>{await enqueue('fix_match',{title:selected.title,artist:selected.artist,album:selected.album||'',plex_id:c.plex_id,playlist_keys:keys});setError('')}}/>}
...
function Detail({playlistKey,latest,enqueue,refresh,jobs}:{playlistKey:string;latest?:Playlist;enqueue:Enqueue;refresh:()=>Promise<void>;jobs:Job[]}){
 ...
 const [stage,setStage]=useState<any>({events:[],percent:0}),[trackFilter,setTrackFilter]=useState('All')
